...
const checkData = [
  {
    q: "?¬ìš©???˜ê? 10ë°??˜ì—ˆ????Client-Server?€ P2P??ë°°í¬ ?œê°„?€ ê°ê° ?´ë–»ê²?ë³€?˜ëŠ”ê°€?\n???¤ë¥¸ê°€?",
    a: "CS: ?œë²„ ?…ë¡œ???©ëŸ‰(Us)?€ ê³ ì •?´ë?ë¡??¬ìš©??N???˜ë©´ ë°°í¬ ?œê°„ NF/Us??? í˜•?¼ë¡œ ì¦ê??œë‹¤.\nP2P: ???¼ì–´ê°€ ?©ë¥˜?˜ë©´ ê·??¼ì–´???…ë¡œ???©ëŸ‰??ì¶”ê??œë‹¤ (Us + NÂ·Ui).\në¶„ìž(NF)?€ ë¶„ëª¨(Us + NÂ·Ui)ê°€ ê°™ì´ ì¦ê??˜ë?ë¡?ë°°í¬ ?œê°„ ì¦ê?ê°€ ?¨ì”¬ ?„ë§Œ?˜ë‹¤."
  },
  {
    q: "P2P?ì„œ ???¼ì–´ê°€ ?Œì¼???¼ë?ë§?ê°€ì§€ê³??ˆì„ ???¤ë¥¸ ?¼ì–´?ê²Œ ì¤????ˆëŠ”ê°€?\n?´ê²Œ ??ì¤‘ìš”?œê??",
    a: "ì¤????ˆë‹¤. BitTorrentì²˜ëŸ¼ ?Œì¼??ì²?¬(chunk) ?¨ìœ„ë¡??˜ëˆ ??êµí™˜?œë‹¤.\n?„ì²´ ?Œì¼????ë°›ê¸° ?„ì—??ë°›ì? ì²?¬ë¥??¤ë¥¸ ?¼ì–´?ê²Œ ?…ë¡œ?œí•  ???ˆë‹¤.\n???Œë¬¸??P2P???¤ìš´ë¡œë“œê°€ ì§„í–‰? ìˆ˜ë¡??„ì²´ ?¤íŠ¸?Œí¬ ?…ë¡œ???©ëŸ‰??ì¦ê??œë‹¤."
  }
];
    ...
    <div className="w-full flex-1 flex flex-col gap-4">
      ...
      <div className="flex-1 flex gap-4">
        ...
        <div className="flex-1 border-2 border-emerald-800 bg-emerald-950/10 rounded-xl p-4 flex flex-col">
          ...
          <div className="font-mono text-xs text-gray-500 mb-3">
            D<sub>p2p</sub> = max(F/U<sub>s</sub>, F/d<sub>min</sub>, NF/(U<sub>s</sub>+Î£U<sub>i</sub>)) = <span className="text-white">{p2pTime}</span> units
          </div>
